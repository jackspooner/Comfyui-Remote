...
Install this repository into `ComfyUI/custom_nodes/Cutlery-Remote`, activate the ComfyUI virtual environment, and install its dependency from the custom-node directory:

```shell
python -m pip install -r requirements.txt
```

The requirements file installs `cutlery-workflow-contracts` from the immutable GitHub commit for v0.1.0. Restart ComfyUI after installation. Cutlery Remote does not require the broader Cutlery Nodes package.
...
Inbound roles are disabled by default. Outbound requests are restricted to explicit configured targets, and bearer tokens are attached only after that trust check. Servers perform constant-time bearer validation, bound transfers, verify file hashes, and promote received files atomically. Generic Remote and Remote CLIP clients cap declared and streamed JSON responses before decoding them. The browser frontend is not a general-purpose origin proxy.
